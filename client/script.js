...
const repeatMyParam = () => {
   axios.get(`http://localhost:3000/repeat/helloWorld`).then(response =>{
       console.log(response.data)
       repeatButton.textContent = response.data
...
axios.get('http://localhost:3000/query-test/hi').then(response =>{
    console.log(response.data)
    queryTest.textContent = response.data

    })

}
queryTest = document.querySelector('#query-button')
queryTest.addEventListener('click', quer)
